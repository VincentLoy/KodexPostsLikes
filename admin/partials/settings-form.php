        ...
        foreach ($this->options as $k => $object):
            ...
            <td>
                <?php if ($object->type == 'checkbox'): ?>
                    <input type="hidden"
                           name="<?php echo $this->plugin_name; ?>[<?php echo $k; ?>]" value="0">
                    <input type="checkbox"
                           name="<?php echo $this->plugin_name; ?>[<?php echo $k; ?>]"
                           value="1" <?php echo ($value) ? 'checked' : ''; ?>>

                <?php elseif ($object->type == 'text'): ?>
                    <input type="text" name="<?php echo $this->plugin_name; ?>[<?php echo $k; ?>]"
                           value="<?php echo esc_attr($value); ?>">

                <?php elseif ($object->type == 'number'): ?>
                    <input type="number"
                           name="<?php echo $this->plugin_name; ?>[<?php echo $k; ?>]"
                           value="<?php echo esc_attr($value); ?>" min="1" max="31" step="1"
                           required>
                ...
                <?php elseif ($object->type == 'array'):
                    foreach ($object->maybe as $p): ?>
                        <label>
                            <input type="checkbox"
                                   name="<?php echo $this->plugin_name; ?>[<?php echo $k; ?>][]"
                                   value="<?php echo esc_attr($p); ?>" <?php echo (in_array($p, $value)) ? 'checked' : ''; ?>>
                            <?php echo $p; ?>
                        </label><br>
                    <?php endforeach; ?>
